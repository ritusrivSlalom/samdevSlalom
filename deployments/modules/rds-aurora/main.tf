locals {
  ...
  postgresql_family = "aurora-postgresql11"
}
...
resource "aws_db_parameter_group" "postgresql" {
  count = var.serverless_enabled == false ? 1 : 0

  name        = "bip-aurora-postgresql"
  family      = local.postgresql_family
  description = format("Parameter group for %s", "bip-aurora-postgresql")
...
resource "aws_rds_cluster_parameter_group" "postgresql" {
  ...
  family      = local.postgresql_family
  description = format("Cluster parameter group for %s", "bip-aurora-postgresql")
