# LDH - proper production place for this module is ./sam-api-gateway-token-auth/cronfunction/handler.py
import json
import sys
import os
import json
...
GOOD_BUCKET_DEST = 'bip-analysis-bucket-dev'        # Mainly for test - remove later
...
# Pass back the location ARNs of the source path (aka nfs) and destination path (aka s3)
# RFU: def create_locations(client, srcbucket, src_folders, destbucket, dest__folders):
...
def handler(event, context):
    ...
    try:
        ...
        for (task_name, src, dest, status, task_id) in rows:
            ...
            if status == 'TASK_CREATION' and task_id is not None and dest == GOOD_BUCKET_DEST :
            ...
            # elif status == 'TASK_CREATION' and task_id is None and dest == GOOD_BUCKET_DEST: ## LDH - testing
